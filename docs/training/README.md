...
本目录用于给“完全不了解这个项目，但需要借助 AI 持续开发这个项目”的同事做培训和日常操作参考。

材料基于仓库在 **2026-03-16** 的实际代码状态整理，不完全依赖旧版 `README.md`。需要先提醒受训同事一点：当前代码已经启用了 `system`、`sales`、`production` 三类模块，旧文档里“只保留系统管理模块”的表述已经落后于代码现状。

## 这套材料解决什么问题

看完并按顺序讲完后，受训同事应该能回答下面 6 个问题：

1. 这个仓库到底是“一个业务项目”还是“一个可扩展基座”。
2. 前端路由、菜单、权限、布局过滤为什么要一起改。
3. 后端菜单树、角色菜单、种子数据为什么会影响前端访问结果。
4. 为什么这里要引入 skill，而不是让 AI 每次自由发挥。
5. 为什么新增模块优先走脚手架和 registry，而不是继续堆主文件。
6. 为什么改完代码后必须跑固定的基座验收链路。

除此之外，这套材料还要解决一个更实际的问题：

“同事拿到这个仓库后，应该怎样把 AI 变成一个能持续接手开发的工程助手，而不是一个只会泛泛解释的聊天工具。”
...
这套仓库的核心不是“把业务页面堆出来”，而是建立一套 **可扩展的业务系统基座 + 可触发的 skill 工作流 + 可复用的脚手架 + 固定的基座验证闭环 + 可持续协作的 AI 开发方式**。
