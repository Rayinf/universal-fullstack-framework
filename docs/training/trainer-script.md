...
本文适合直接照着讲。  
默认对象是“完全不了解本框架，但接下来要借助 AI 持续开发企业管理系统”的同事。
...
> 我们今天学习这个基座为什么这样设计，以及以后怎么在它上面稳定扩展。
...
> 在这个仓库里，AI 的用法是：先进入项目上下文，再沿着基座、skill、脚手架、registry、验证这条链路工作。
...
> 在这个项目里，页面访问至少要同时满足：模块启用、菜单可见、角色授权、权限码一致。
> 新增模块通常会联动多个文件。
...
2. `skills/universal-fullstack-framework/SKILL.md`
3. `skill-creator` 的 `SKILL.md`（由当前 Agent 环境提供）

### 7.3 建议讲法

> 基座定义了系统结构，skill 定义了操作这个结构的标准方法。
> 扩展和排错都有固定路径。
...
演示建议在临时分支或培训专用仓库副本中进行，保持主开发分支历史干净。
...
菜单可见属于展示状态，页面访问还要看守卫里按 `functionCode / permission / menuId` 的最终判定。

### 11.3 为什么要保留 registry
...
skill 负责给出方法，脚手架负责执行生成，二者是工作流上下游关系。
