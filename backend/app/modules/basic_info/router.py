from __future__ import annotations

from typing import Any, Callable

from fastapi import APIRouter

from app.modules.basic_info.deps import BasicInfoRouterDeps
from app.modules.basic_info.routes.basic_info_routes import register_basic_info_routes


def create_basic_info_router(
  ok_func: Callable[[Any, str], dict[str, Any]],
  fail_func: Callable[[str, int], Any],
  get_conn_func: Callable[[], Any],
  now_str_func: Callable[[], str],
  safe_int_func: Callable[[Any, int], int],
) -> APIRouter:
  router = APIRouter(tags=['基础参数'])
  deps = BasicInfoRouterDeps(
    ok_func=ok_func,
    fail_func=fail_func,
    get_conn_func=get_conn_func,
    now_str_func=now_str_func,
    safe_int_func=safe_int_func,
  )
  register_basic_info_routes(router, deps)
  return router
