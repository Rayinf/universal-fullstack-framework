...
def fetch_user_ids_by_name_or_username(cur: Any, applicant_name: str) -> list[str]:
  cur.execute('SELECT user_id FROM users WHERE real_name = ? OR username = ?', (applicant_name, applicant_name))
  return [str(row['user_id']) for row in cur.fetchall()]
