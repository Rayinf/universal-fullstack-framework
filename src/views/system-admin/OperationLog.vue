  ...
  <div class="page-view">
    <div class="page-header">
      <div class="title-container">
        ...
        <div class="title-row">
          <h2>操作日志管理</h2>
          <div class="page-description">查看和管理系统操作日志，支持日志查询、删除和清空。</div>
        </div>
      </div>
    </div>
    ...
    <div class="content-card">
      <div class="search-actions-panel">
        <el-form :model="query" class="search-form" inline @submit.prevent>
      ...
      <div class="table-container">
        <div class="table-header">
          ...
          <div class="table-header__actions">
            ...
            <el-button type="danger" @click="handleClearLogs" :disabled="!hasData" plain>
        ...
        >
          ...
          <el-table-column label="操作" width="140" align="center" class-name="col-actions">
            <template #default="{ row }">
...
import { Delete, User, Search, RefreshLeft } from '@element-plus/icons-vue'
import {
  pageOperationLogApi,
  getOperationLogDetailApi,
  deleteSysLogUserApi,
  batchDeleteSysLogUserApi,
  clearSysLogUserApi,
} from '@/api/system/log'
import type {
  OperationLogRecord,
...
const loadData = async () => {
  ...
  try {
    ...
    const queryDto: SysLogUserQueryDto = {
      ...
      logType: query.logType, // 使用用户选择的日志类型
    ...
    if (response.code === 0 && response.data) {
      ...
      logs.value = response.data.records.map(
        (record: SysLogUserVO): OperationLogRecord => ({
          // 原始API字段
          id: record.id,
...
const extractModuleFromContent = (content: string): string => {
  if (content.includes('用户') || content.includes('登录') || content.includes('账号'))
    return '用户管理'
  if (content.includes('订单') || content.includes('项目') || content.includes('工单'))
    return '订单管理'
  if (content.includes('客户') || content.includes('顾客')) return '客户管理'
  if (content.includes('文档') || content.includes('资料') || content.includes('文件'))
    return '文档管理'
  if (content.includes('系统') || content.includes('配置') || content.includes('参数'))
    return '系统管理'
  if (content.includes('备份') || content.includes('数据库') || content.includes('恢复'))
    return '数据备份'
  return '其他'
}
...
const handleDelete = async (row: OperationLogRecord) => {
  try {
    await ElMessageBox.confirm(`确定要删除ID为 ${row.id} 的日志吗？此操作不可撤销。`, '删除确认', {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const response = await batchDeleteSysLogUserApi(ids)
    if (response.code === 0) {
...
const handleConfirmClear = async () => {
  ...
  try {
    const response = await clearSysLogUserApi({
      logType: clearForm.logType,
      clearType: clearForm.clearType,
    })
    if (response.code === 0) {
