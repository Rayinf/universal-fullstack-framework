...
import type { SysDept as APISysDept } from '@/types/system/dept'
...
export const useDeptStore = defineStore('dept', () => {
  ...
  const convertTreeNode = (node: any): DeptTreeNode => {
    ...
    const isPerson = String(node.type) === '1'
    ...
    // 基于实际构建的子节点计算数量，不依赖 API 返回的可能不准确的字段
    let totalUserCount = 0
    let childDeptCount = 0
    ...
    if (result.children && result.children.length > 0) {
      result.children.forEach((child) => {
        if (child.type === 1) {
          ...
          childDeptCount += 1
          totalUserCount += child.userCount || 0
        }
      })
    }

    result.userCount = totalUserCount
    result.childDeptCount = childDeptCount
  ...
  const findTreeNode = (
    id: string,
    nodes: DeptTreeNode[] = deptTree.value,
  ): DeptTreeNode | null => {
  ...
  const fetchDeptById = async (id: string) => {
    ...
    try {
      ...
      if (response.code === 0 && response.data) {
        ...
        // 从树形数据中获取准确的成员数量和子组织数量
        const treeNode = findTreeNode(String(deptData.deptId))
        if (treeNode) {
          deptData.userCount = treeNode.userCount ?? 0
          deptData.childDeptCount = treeNode.childDeptCount ?? 0
        }
  ...
  const fetchDeptByName = async (deptName: string) => {
    loading.value = true
    try {
      const response = await request.get<SysDept>(
        `/admin/dept/details/${encodeURIComponent(deptName)}`,
      )
  ...
  const flatDeptList = computed(() => {
    const result: { id: string; name: string; parentId: string; level: number; type: number }[] = []
