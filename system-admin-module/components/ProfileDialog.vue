  ...
  >
        ...
        <div class="profile-info">
          ...
          <div class="info-item">
            ...
            <div class="roles">
              ...
              <span v-if="!user?.roles?.length" class="no-data">未设置</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="profile-dialog__footer">
      ...
      <template v-else>
        ...
        <el-button type="primary" size="small" @click="handlePasswordSubmit" :loading="passwordLoading">
          确认修改
        </el-button>
      </template>
    </div>
...
const user = computed(() => {
  console.log('ProfileDialog中的用户信息:', authStore.userInfo)
...
const loadRoleNames = async () => {
  try {
    const res = await request<any>({ url: '/admin/role/list', method: 'get' })
    if ((res.code === 0 || res.code === 200) && Array.isArray(res.data)) {
      const map: Record<string, string> = {}
      res.data.forEach((r: any) => {
        if (r.roleId != null) map[String(r.roleId)] = r.roleName
      })
      roleNameMap.value = map
    }
  } catch (e) {
    console.error('加载角色名称失败:', e)
  }
}

const getRoleName = (roleId: string): string => {
  return roleNameMap.value[roleId] || `角色${roleId}`
}
