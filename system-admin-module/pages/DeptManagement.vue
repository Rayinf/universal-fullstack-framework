  ...
  <div class="dept-management-view page-view">
    ...
    <div class="page-header">
      <div class="title-container">
        ...
        <div class="title-row">
          <h2>组织架构管理</h2>
          <div class="page-description">管理公司的组织架构体系，包括创建、编辑、删除部门，以及管理部门层级关系</div>
        </div>
      </div>
    </div>
      ...
      <div class="dept-layout">
        ...
        <div class="tree-panel">
          <div class="tree-header">
            <el-input v-model="searchKeyword" placeholder="搜索部门或成员" clearable class="search-input"
              @input="handleSearch">
              <template #prefix>
                <el-icon>
                  <Search />
            ...
            <el-button type="primary" @click="handleAddRootDept" :icon="Plus" circle size="small" title="添加顶级部门" />
          </div>

          <div class="tree-content" v-loading="deptStore.loading">
            <el-tree ref="treeRef" :data="deptStore.deptTree" :props="{ label: 'name', children: 'children' }"
              node-key="id" :expand-on-click-node="false" :filter-node-method="filterNode" class="dept-tree"
              ...
              <template #default="{ node, data }">
                <div class="tree-node">
                  ...
                  <div v-if="data.type === 0" class="node-actions">
                    <el-button type="primary" :icon="Plus" size="small" circle @click.stop="handleAddChildDept(data)"
                      title="添加子部门" />
                    <el-button type="warning" :icon="Edit" size="small" circle @click.stop="handleEditDept(data)"
                      title="编辑部门" />
                    <el-button type="danger" :icon="Delete" size="small" circle @click.stop="handleDeleteDept(data)"
                      title="删除部门" :disabled="!deptStore.canDeleteDept(data)" />
                  </div>
                </div>
              </template>
            </el-tree>
        ...
        <div class="detail-panel">
          <div v-if="selectedDept" class="dept-detail">
            <div class="detail-header">
              ...
              <div class="detail-actions">
                <el-button type="primary" @click="handleEditDept(selectedDept)" :icon="Edit">编辑部门</el-button>
                <el-button type="danger" @click="handleDeleteDept(selectedDept)" :icon="Delete"
                  :disabled="!deptStore.canDeleteDept(selectedDept)">删除部门</el-button>
              </div>
            </div>
            ...
            <div class="detail-content">
              ...
              <div v-if="selectedDept.userList && selectedDept.userList.length > 0" class="members-section">
                <h4>部门成员</h4>
                <div class="members-list">
                  <div v-for="user in selectedDept.userList" :key="user.userId" class="member-item"
          ...
          <div v-else class="empty-detail">
            <el-empty description="请选择部门或成员查看详情" />
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 部门表单对话框 -->
    <el-dialog v-model="deptDialogVisible" :title="dialogTitle" width="600px" destroy-on-close
      ...
      <el-form ref="deptFormRef" :model="deptForm" :rules="deptFormRules" label-width="100px">
        ...
        <el-form-item label="上级部门" prop="parentId">
          <el-tree-select v-model="deptForm.parentId" :data="parentTreeOptions" :props="{ label: 'name', value: 'id' }"
            placeholder="请选择上级部门（不选则为顶级部门）" clearable check-strictly :render-after-expand="false" style="width: 100%" />
        </el-form-item>
        <el-form-item label="排序" prop="sortOrder">
          <el-input-number v-model="deptForm.sortOrder" :min="0" :max="9999" placeholder="数字越小越靠前" />
        </el-form-item>
        <el-form-item label="部门成员">
          <el-tree-select v-model="deptForm.userIdList" :data="deptStore.userTreeData"
            :props="{ label: 'label', value: 'value', disabled: 'disabled' }" multiple placeholder="请选择部门成员"
            clearable check-strictly :render-after-expand="false" style="width: 100%" v-loading="deptStore.userListLoading" />
        </el-form-item>
    ...
    <el-dialog v-model="deleteDialogVisible" title="删除确认" width="500px" destroy-on-close>
      ...
      <div class="delete-info">
        <p v-if="deleteTargetDept?.childDeptCount && deleteTargetDept.childDeptCount > 0">
          ...
          子部门：{{ deleteTargetDept.childDeptCount }} 个
        </p>
        <p v-if="deleteTargetDept?.userCount && deleteTargetDept.userCount > 0">
          <el-icon>
            <User />
          </el-icon>
          部门成员：{{ deleteTargetDept.userCount }} 人
        </p>
      </div>
      <template #footer>
...
import { useDeptStore, type DeptTreeNode, type SysDept } from '../stores/modules/dept'
...
const dialogTitle = computed(() => {
  return formType.value === 'add' ? '添加部门' : '编辑部门'
})
...
const parentTreeOptions = computed(() => {
  const filterTree = (nodes: DeptTreeNode[]): DeptTreeNode[] => {
    return nodes
      ...
      .map((node) => ({
        ...node,
        children: node.children ? filterTree(node.children.filter(child => child.type === 0)) : undefined,
      }))
  }
...
const handleUserClick = (user: any) => {
  ...
  selectedUser.value = {
    ...
    parentId: selectedDept.value?.deptId || '',
    type: 1,
    userId: user.userId,
...
const handleDeleteDept = async (dept: SysDept | DeptTreeNode) => {
  ...
  ElMessageBox.confirm(
    `确定要删除部门"${deptDetail.name}"吗？此操作不可恢复。`,
    '删除确认',
    {
